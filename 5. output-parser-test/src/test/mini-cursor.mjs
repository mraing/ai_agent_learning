...
`));
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    // 必须是 new Map()：newMap() 是未定义的函数，会在进入循环前直接抛 ReferenceError
    ...
    for await (const chunk of rawStream) {
      ...
      if (parsedTools && parsedTools.length > 0) {
        ...
        // 注意：deepseek-flash 是 thinking 模型，思维链通过 additional_kwargs.reasoning_content
        // 下发（此时 chunk.content 为空），这里必须只判断 content，否则会把思维链打到终端
        if (chunk.content) {
