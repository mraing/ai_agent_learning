...
import { JobAgentService } from '../ai/job-agent.service.js';
...
export class JobService implements OnApplicationBootstrap {
  ...
  @Inject(forwardRef(() => JobAgentService))
  private readonly jobAgentService: JobAgentService;

  /** 应用启动时，把数据库里已启用的任务重新注册到调度器（服务重启不丢任务） */
  async onApplicationBootstrap() {
    ...
    for (const job of enabledJobs) {
      ...
      try {
        ...
        this.logger.error(
          `恢复任务 ${job.id} 失败：${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }

    if (enabledJobs.length) {
      this.logger.log(`已恢复 ${enabledJobs.length} 个定时任务`);
    }
  }
  ...
  /** 真正执行任务：跑一个 agent loop 去解析并执行 instruction */
  private async runJobInstruction(job: Job) {
    this.logger.log(`run job ${job.id}, ${job.instruction}`);

    try {
      const result = await this.jobAgentService.runJob(job.instruction);
      this.logger.log(`job ${job.id} 执行完成：${result}`);
    } catch (error) {
      this.logger.error(
        `job ${job.id} 执行失败：${error instanceof Error ? error.message : String(error)}`,
      );
    } finally {
  ...
  private async startRuntime(job: Job) {
    ...
    if (job.type === 'every') {
      ...
      const ref = setInterval(() => {
        void this.runJobInstruction(job);
      }, job.everyMs);
    ...
    if (job.type === 'at') {
      ...
      const delay = Math.max(0, new Date(job.at).getTime() - Date.now());
      const ref = setTimeout(async () => {
        await this.runJobInstruction(job);
        // at 类型只执行一次：执行完自动停用
