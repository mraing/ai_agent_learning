...
export class DbUsersCrudToolService {
  ...
  constructor(private readonly usersService: UsersService) {
    ...
    // UsersService 在查不到时会抛 NotFoundException，这里统一转成 null，方便给出友好提示
    const findUserOrNull = async (id: number): Promise<User | null> => {
      try {
      ...
      async ({
        ...
        switch (action) {
          ...
          case 'get': {
            ...
            const user = await findUserOrNull(id);
            if (!user) {
          ...
          case 'update': {
            ...
            const existing = await findUserOrNull(id);
            if (!existing) {
              return `ID 为 ${id} 的用户在数据库中不存在。`;
            }
