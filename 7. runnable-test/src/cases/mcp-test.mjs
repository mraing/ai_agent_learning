    ...
    func: async (input) => {
        ...
        for (const toolCall of response.tool_calls ?? []) {
            ...
            try {
                ...
                // 工具调用失败（如 API 配额超限）时，把错误作为 ToolMessage 反馈给模型，让它调整策略继续，而不是中断整个 agent 循环
                toolResults.push(new ToolMessage({
                    content: `工具 ${toolCall.name} 调用失败: ${error.message}`,
...
try {
    ...
    // 正常/异常退出时都关闭 MCP 连接，避免 chrome-devtools 等子进程残留
